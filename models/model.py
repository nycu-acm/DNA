...
class SDT_Generator(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=2, num_head_layers= 1,
        ...
        ### content ecoder to loss
        self.stage = stage
        if stage == '1':
            self.cls_head = nn.Linear(d_model, cls_num)
    ...
    def forward(self, style_imgs, seq, char_img, char_com, char_struct):
        ...
        if self.stage == '1':
            return pred_sequence, nce_emb, nce_emb_patch, char_cls, seq_cls
        else:
            return pred_sequence, nce_emb, nce_emb_patch

    # style_imgs: [B, N, C, H, W]
    def inference(self, style_imgs, char_img, char_com, char_struct, max_len):
        batch_size, num_imgs, in_planes, h, w = style_imgs.shape
        # [B, N, C, H, W] -> [B*N, C, H, W]
        ...
        char_emb, _ = self.mixer(char_emb, char_emb_c)
