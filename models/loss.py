...
def get_pen_loss(z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr, z_pen_logits, x1_data, x2_data,
                 pen_data):
    result0 = tf_2d_normal(x1_data, x2_data, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr) # [T, 20]
    epsilon = 1e-10
    # result1 is the loss wrt pen offset
    result1 = torch.multiply(result0, z_pi) # [T, 20]
    result1 = torch.sum(result1, 1, keepdims=True) # [T, 1]
    result1 = - torch.log(result1 + epsilon)  # avoid log(0) # [T, 1]
