...
script={"CHINESE":['CHINESE', 'dictionary_ids.txt']
        }

class ScriptDataset(Dataset):
    def __init__(self, root='data', dataset='CHINESE', is_train=True, num_img = 15, test_dataset=None, split=False):
        data_path = os.path.join(root, script[dataset][0])
        self.content = pickle.load(open(os.path.join(root, 'src/src.pkl'), 'rb')) #content samples

        self.is_train = is_train
        self.split = split

        self.char_dict = pickle.load(open(os.path.join(data_path, 'character_dict_t_only.pkl'), 'rb'))
        self.char_dict2 = pickle.load(open(os.path.join(data_path, 'character_dict_t_only_all.pkl'), 'rb'))

        ###/ mix
        with open(os.path.join(data_path, script[dataset][1]), 'r') as f:
            self.compo_dict = json.loads(f.readline().replace('\'', '\"'))

        ###/    seq
        with open(os.path.join(data_path, 'ids.json'), 'r') as f:
            self.component = json.load(f)
        with open(os.path.join(data_path, 'compo_to_idx.json'), 'r') as f:
            self.compo2idx = json.load(f)
        
        if split:
            self.all_writer = pickle.load(open(os.path.join(data_path, 'writer_dict_t_split.pkl'), 'rb'))
        else:
            self.all_writer = pickle.load(open(os.path.join(data_path, 'writer_dict_t.pkl'), 'rb'))

        if self.is_train:
            lmdb_path = os.path.join(data_path, 'train_tra_all_json_eli') # online characters
            self.img_path = os.path.join(data_path, 'train_tra_all_img') # style samples
        ...
        with self.lmdb.begin(write=False) as txn:
            ...
            if self.max_len <= 0:
                ...
                for i in range(self.num_sample):
                    ...
                    tag_writer = pickle.loads(data_byte)['fname']
                    tag_coord = pickle.loads(data_byte)['coordinates']
                    if tag_coord.shape[0] > 1:
                        if tag_char in self.char_dict and test_dataset != 1:
                            if self.split:
                                if tag_writer in self.writer_dict:
                                    self.indexes.append(i)
                            else:
                                self.indexes.append(i)
                        elif tag_char not in self.char_dict and test_dataset == 1:
                            if self.split:
                                if tag_writer in self.writer_dict:
                                    self.indexes.append(i)
                            else:
    ...
    def __getitem__(self, index):
        with self.lmdb.begin(write=False) as txn:
            data = pickle.loads(txn.get(str(self.indexes[index]).encode('utf-8')))
            tag_char, coords, fname = data['tag_char'], data['coordinates'], data['fname']

        ###/ mix
        char_com = self.compo_dict['valid'][tag_char]['components']
        char_struct = [self.compo_dict['valid'][tag_char]['struct']]
        ...
        ###/    seq
        char_decom = self.component[tag_char]['decompo']
        char_decom_idx = [self.compo2idx[c] for c in char_decom]
        char_decom_idx += ([0] * (1 * (32 > len(char_decom_idx))))
        char_decom_idx += ([-1] * (32 - 1 - len(char_decom_idx)))

        writer_id = self.writer_dict[fname]

        character_id1 = self.char_dict.find(tag_char)
        character_id = self.char_dict2.find(tag_char)
        label_id = []
        for i in range(self.num_img):
            label_id.append(self.char_dict2.find(img_label[i]))
        return {'coords': torch.Tensor(coords),
                'character_id': torch.Tensor([character_id]),
                'character_id1': torch.Tensor([character_id1]),
                ...
                'char_decom_idx': torch.Tensor(char_decom_idx)}
    ...
    def collate_fn_(self, batch_data):
        ...
        for i in range(bs):
            ...
            output['character_id1'][i] = batch_data[i]['character_id1']
            output['writer_id'][i] = batch_data[i]['writer_id']
            output['img_list'].append(batch_data[i]['img_list'])
            output['char_com'].append(batch_data[i]['char_com'])
            output['char_struct'].append(batch_data[i]['char_struct'])
            output['char_img'].append(batch_data[i]['char_img'])
            output['img_label'].append(batch_data[i]['img_label'])
            output['char_decom_idx'].append(batch_data[i]['char_decom_idx'])
        output['img_list'] = torch.stack(output['img_list'], 0) # -> (B, num_img, 1, H, W)
        output['char_com'] = torch.stack(output['char_com'], 0)
        output['char_struct'] = torch.stack(output['char_struct'], 0)
        ...
        output['char_decom_idx'] = torch.stack(output['char_decom_idx'], 0)
        return output
