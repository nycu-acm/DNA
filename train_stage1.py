...
def main(opt):
    ...
    test_dataset = ScriptDataset(
       cfg.DATA_LOADER.PATH, cfg.DATA_LOADER.DATASET, cfg.TEST.ISTRAIN, cfg.MODEL.NUM_IMGS, split=True)
    ...
    model = SDT_Generator(num_encoder_layers=cfg.MODEL.ENCODER_LAYERS,
            ...
            gly_dec_layers= cfg.MODEL.GLY_DEC_LAYERS, stage='1').to('cuda')
    ### load checkpoint
    ...
    if len(opt.content_pretrained) > 0:
        ...
        if len(pretrained_dict) > 0:
            model_dict.update(pretrained_dict)
            model.load_state_dict(model_dict)
            print('load pretrained model from {}'.format(opt.content_pretrained))
    else:
        pass
    ...
    trainer = Trainer(model, criterion, optimizer, train_loader, logs, char_dict, test_loader, stage='1')
    trainer.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log', default='stage1',
